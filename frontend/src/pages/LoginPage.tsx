...
export default function LoginPage() {
  ...
  const validate = () => {
    ...
    if (!validateEmail(form.email)) next.email = "Ingresa un correo electronico valido.";
    if (!form.password) next.password = "La contrasena es obligatoria.";
    if (mode === "register") {
      if (form.name.trim().length < 2) next.name = "El nombre debe tener minimo 2 caracteres.";
      if (!validatePhone(form.phone)) next.phone = "Ingresa un telefono valido, solo numeros, +, espacios o guiones.";
      if (!validatePassword(form.password)) next.password = "Minimo 8 caracteres, mayuscula, minuscula, numero y simbolo.";
      if (form.password !== form.confirmPassword) next.confirmPassword = "Las contrasenas no coinciden.";
    }
    setErrors(next);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        setMode("login");
        setForm(prev => ({ ...initialForm, email: prev.email, type: prev.type }));
      }
    } catch (err) {
      const apiError = err as ApiError;
      const fieldErrors: Record<string, string> = {};
      if (apiError.errors) Object.entries(apiError.errors).forEach(([key, value]) => { fieldErrors[key] = value?.[0] || "Campo invalido"; });
  ...
  return <main className="page">
    <section className="auth-card">
      ...
      <form onSubmit={submit} noValidate>
        {mode === "register" && <>
          <Field label="Tipo de registro" icon={<Users size={16}/>}>
            <select value={form.type} onChange={e => setField("type", e.target.value as RegisterType)}>
          ...
          <Field label="Telefono" icon={<Phone size={16}/>} error={errors.phone}><input value={form.phone} onChange={e => setField("phone", e.target.value)} placeholder="3001234567" /></Field>
        </>}

        <Field label="Correo electronico" icon={<Mail size={16}/>} error={errors.email}><input value={form.email} onChange={e => setField("email", e.target.value)} placeholder="communifield@gmail.com" autoComplete="email" /></Field>

        <Field label="Contrasena" right={mode === "login" ? "No la recuerdas?" : undefined} icon={<Lock size={16}/>} error={errors.password}>
        ...
        {mode === "register" && <Field label="Confirmar contrasena" icon={<Lock size={16}/>} error={errors.confirmPassword}><input type="password" value={form.confirmPassword} onChange={e => setField("confirmPassword", e.target.value)} placeholder="********" /></Field>}

        {mode === "login" && <label className="remember"><input type="checkbox" checked={form.remember} onChange={e => setField("remember", e.target.checked)} /> Mantener la sesion iniciada</label>}
      ...
      <p className="swap">{mode === "login" ? "Eres nuevo en esto?" : "Ya tienes cuenta?"} <button onClick={() => { setMode(mode === "login" ? "register" : "login"); setAlert(null); setErrors({}); }}>{mode === "login" ? "Crear cuenta" : "Iniciar sesion"}</button></p>
